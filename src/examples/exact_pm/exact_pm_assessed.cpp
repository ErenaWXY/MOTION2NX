...
static long get_rss_kb() {
  ...
  std::string k, v;
  while (f >> k >> v) {
    if (k == "VmRSS:") return std::stol(v); // kB
  }
  return -1;
}
...
static std::optional<Options> parse_cli(int argc, char** argv) {
  ...
  desc.add_options()
    ...
    // generator
    ("alpha-patterns", po::bool_switch()->default_value(true), "Use A-Za-z generator")
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    print_phase("secret_share", secret_share_dur, {/*no comm in local prep*/0,0}, rss0);
